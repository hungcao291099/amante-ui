import styles from "@components/ConceptRoom/Write/Write.module.css"
import styles2 from "@styles/ConceptRoomWrite.module.css"
import { uploadSingle, deleteSingle, deleteMultiple } from "@apis/upload.api.jsx"
...
import { productOptions } from "@utils/writeUtils"
...
export default ({ data, mode, selectedView }) => {
    ...
    const TIME_5_MIN = 300000

    const productQuery = useQuery("products", getProduct, { cacheTime: TIME_5_MIN, staleTime: TIME_5_MIN })
    ...
    useEffect(() => {
        if (productQuery.isFetched && !productQuery.isError) {
            setProducts(productOptions(productQuery.data.response, productQuery.isFetched))
        }
    }, [productQuery.data])
        ...
        try {
            switch (type) {
                case "p_light":
                    ...
                    formPLight.append("image", e.target.files[0])
                ...
                case "o_thumb":
                    ...
                    formOThumb.append("image", e.target.files[0])
    ...
    const handleSaveObj = () => {
        if (tempObj.option_file_nm !== "" && tempObj.thumbnail_img !== "" && tempObj.product.length) {
            if (isEditObj == "") {
    ...
    const handleObjectInput = async (type, e) => {
        switch (type) {
            case "c_img":
                ...
                cImgForm.append("image", e.target.files[0])
            ...
            case "o_img_l":
                ...
                oImgForm.append("image", e.target.files[0])
            ...
            case "o_img_n":
                ...
                oImgForm1.append("image", e.target.files[0])
                            ...
                            </label>
                            {
                                ...
                                (<div className={styles.object__card}>
                                    ...
                                    <div className={styles.room__input__group2}>
                                        <label className={styles.label}>옵션명</label>
